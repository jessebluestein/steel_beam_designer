...
iLoc = 0 * units.ft
jLoc = 13 * units.ft
lineLoad1 = LineLoad(iLoc = iLoc, jLoc = jLoc, iLineLoad = w0mag, jLineLoad = w0mag, type = 'D', desc = 'floor')
lineLoad2 = LineLoad(iLoc = iLoc, jLoc = jLoc, iLineLoad = w1mag, jLineLoad = w1mag, type = 'Lr', desc = 'floor')
lineLoads = [lineLoad1, lineLoad2]

beam = SteelBeam(nodes, rawDistLoads = lineLoads, considerSelfWeight = True,
depthClass = 10, weightClass = 12, eleSpacing = 2 * units.inch,
bendingAxis = 'strong', outputPDF = True, name = 'Test3', outputPath = 'output_reports/test3')
