...
class SteelBeam(Beam):
    ...
    class Flange():
        ...
        class lamFlange():
            ...
            def updateFactor(self):
                ...
                return sf.makeEquation('\\lambda_{{flange}}', 'b / t',
                {'b': round(b, 2), 't': round(t, 4)}, self.val)
        ...
        class lamPflange():
            ...
            def updateFactor(self):
                ...
                self.ref = f'AISC/ANSI 360-{self.flange.beam.specVersion} Table B4.1b'
                return f'\\lambda_{{P-flange}} = {self.flange.beam.makeE_FyString(0.38)}'
        ...
        def checkCompactness(self):
            ...
            return(sf.fixUnits(f'{comparison} _arrow_ \; _bf_{{{self.compactness} Flange}}', type = 'text'))
    ...
    class Lp():
        ...
        def updateFactor(self):
            ...
            self.val = (1.76 * self.beam.ry * (self.beam.E / self.beam.Fy)**(1/2)).to(units.ft)
            self.ref = f'AISC/ANSI 360-{self.beam.specVersion} Eq. F2-5'
            return sf.makeEquation('L_p', '1.76 *mul* r_y *mul* sqrt(E/F_y)',
            {'r_y': self.beam.ry, 'E': self.beam.E, 'F_y': self.beam.Fy}, self.val)
    ...
    class Lr():
        ...
        def updateFactor(self):
            ...
            Sx = self.beam.Sx
            h0 = self.beam.h0
            val0 = (((J*c)/(Sx*h0))**2 + 6.76*((0.7*Fy)/E)**2)**(1/2)
            val1 = ((J*c)/(Sx*h0) + val0)**(1/2)
            self.val = (1.95 * rts * (E/(0.7*Fy)) * val1).to(units.ft)
            self.ref = f'AISC/ANSI 360-{self.beam.specVersion} Eq. F2-6'
            return sf.makeEquation('L_r', '1.95 *mul* r_ts *mul* (E/(0.7 *mul* F_y)) * sqrt((J *mul* c)/(S_x *mul* h_0) + sqrt(((J *mul* c)/(S_x *mul* h_0))**2 + 6.76 * ((0.7 *mul* F_y)/E)**2))', {'F_y': Fy, 'E': E, 'J': round(self.beam.J, 2), 'c}': f'{self.c}}}', 'S_x': round(Sx, 1), 'h_0': round(h0, 1), 'r_{ts}': round(rts, 1)}, self.val, lineBreak = 'True')
    ...
    class Mp():
        ...
        def updateFactor(self):
            ...
            return sf.makeEquation('M_p', 'F_y *mul* Z_x', {'F_y': self.beam.Fy, 'Z_x': round(self.beam.Zx, 1)}, self.val.to(self.beam.outUnit['M']))
        ...
        class phiV():
            ...
            def updateFactor(self):
                ...
                if h/tw <= 2.24 * (E/Fy)**(1/2):
                    comparison = '<='
                    self.val = 1.0
                    self.ref = f'AISC/ANSI 360-{self.phiVn.beam.specVersion} {{\\S}}G2.1.a'
                else:
                    comparison = '>'
                    self.val = 0.9
                    self.ref = f'AISC/ANSI 360-{self.phiVn.beam.specVersion} {{\\S}}G1.a'
    ...
    class rts():
        ...
        def updateFactor(self):
            """Get/update value and reference."""
            self.val = (((self.beam.Iy * self.beam.Cw)**(1/2))/self.beam.Sx)**(1/2)
            self.ref = f'AISC/ANSI 360-{self.beam.specVersion} Eq. F2-7'
            return sf.makeEquation('r_{{ts}}', 'sqrt(sqrt(I_y *mul* C_w)/S_x)',
            {'I_y': self.beam.Iy, 'C_w': self.beam.Cw, 'S_x': round(self.beam.Sx, 2)}, self.val)
    ...
    class Web():
        ...
        class lamWeb():
            ...
            def updateFactor(self):
                ...
                return sf.makeEquation('\\lambda_{{web}}', 'h/t_w',
                {'h': round(self.web.beam.h, 2), 't_w': round(self.web.beam.tw, 3)}, round(self.val, 1))
        ...
        def checkCompactness(self):
            ...
            return sf.fixUnits(f'{comparison} _arrow_ \; _bf_{{{self.compactness} Web}}', type = 'text')
    ...
    def calcDCRs(self):
        ...
        for node in self.nodes:
            node.bendingDCR = (abs(node.MuMaxAbs['val']) / node.phiMn).to_reduced_units().magnitude
            node.shearDCR = (abs(node.VuMaxAbs['val']) / self.phiVn.val).to_reduced_units().magnitude
    ...
    def checkBending(self):
        ...
        phiMn = round(self.maxMomentNode.unbracedSpan.phiMn.val.to(self.outUnit['M']), 1)
        return sf.fixUnits(f'_bf_{{|M_u| = {moment} _sp_ {comparison} \\phi_b \\cdot M_n = {phiMn} _sp_ (DCR = {round(self.maxMomentNode.bendingDCR, 2)} - {self.bendingCheck})}}')
    ...
    def checkShear(self):
        ...
        return sf.fixUnits(f'_bf_{{|V_u| = {shear} _sp_ {comparison} \\phi_v \\cdot V_n = {phiVn} _sp_ (DCR = {round(self.maxShearNode.shearDCR, 2)} - {self.shearCheck})}}')
    ...
    def makeE_FyString(self, scalar):
        ...
        val = str(round((scalar * (self.E/self.Fy)**(1/2)).to_reduced_units().magnitude, 1))
    ...
    def outputPDFreport(self):
        ...
        for distLoad in self.rawDistLoads:
            ...
            distLoads.append({'startLoc': startLoc, 'startMag': startMag, 'endLoc': endLoc, 'endMag': endMag, 'type': distLoad.type, 'desc': distLoad.desc})

        # reference design vals table content
        props = ['Aw', 'Cw', 'Fu', 'Fy', 'Ix', 'Iy', 'Sx', 'Sy', 'Zx', 'Zy',
        'rx', 'ry', 'bf', 'tf', 'tw', 'h0', 'unitWt']
        refDesignVals = {key: val for key, val in vars(self).items() if key in props}
        refDesignValsTable = []
        for prop in props:
            if len(prop) == 2:
                refDesignValsTable.append(sf.fixUnits(f'{prop[0]}_{{{prop[1]}}} & {round(refDesignVals[prop], 1)}', type = 'text'))
        refDesignValsTable.append(sf.fixUnits(f"U.W. & {round(refDesignVals['unitWt'], 1)}", type = 'text'))
        ...
        for support in self.supports:
            ...
            if support.condition == 'fix':
            	...
            	for idx, type in enumerate(self.loadTypesSub):
            		reaction += f'{round(support.rawMrxns[type].to(self.outUnit["M"]), 1)}'
            		if idx < int(len(self.loadTypesSub) - 1):
            			reaction += ' & '
            		else:
            			reaction += '\\\\ \r'
        ...
        for lc in self.strengthCombos:
        	strengthCombos.append({'lc': sf.fixUnits(lc.name.replace('Lr', f'L_r'),
            type = 'text'), 'ref': sf.fixUnits(lc.ref, type = 'text')})

        deflCombos = []
        for lc in self.deflCombos:
        	deflCombos.append({'lc': sf.fixUnits(lc.name.replace('Lr', f'L_r'),
            type = 'text'), 'ref': sf.fixUnits(lc.ref, type = 'text')})
    ...
    def plotCb(self):
        ...
        for idx, loc in enumerate(labelLocs):
            Cb = round(self.unbracedSpans[idx].Cb.val[self.maxMomentNode.MuMaxAbs['combo']].magnitude, 2)
            ax.text(loc, -0.025, f'C\\textsubscript{{b}} = {Cb}', bbox=dict(boxstyle="round4,pad=.5", fc="0.8"), ha='center', va='center')
    ...
    def setMaterialSpecificSystem(self):
        ...
        if self.bendingAxis == 'strong':
            ...
            self.I = self.Iy
            self.S = self.Sy
            sys.exit('ERROR: Only strong axis bending currently supported for steel beams')
        else:
            sys.exit("ERROR: Invalid bending axis. Enter 'strong' for strong axis bending or 'weak' for weak axis bending.")
        self.A = self.A
    ...
    def setRefDesignValues(self):
        ...
        geoPropUnits = {'A': units.inch**2, 'bf': units.inch, 'Cw': units.inch**6,
        'd': units.inch, 'Ix': units.inch**4, 'Iy': units.inch**4,
        'J': units.inch**4, 'rx': units.inch, 'ry': units.inch,
        'Sx': units.inch**3, 'Sy': units.inch**3, 'tf': units.inch,
        'tw': units.inch, 'Zx': units.inch**3, 'Zy': units.inch**3,
        'kdes': units.inch}
