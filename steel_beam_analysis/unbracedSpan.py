...
class UnbracedSpan:
    ...
    class Cb():
        ...
        def updateFactor(self):
            ...
            if self.unbracedSpan.cantilever:
                ...
                for lc in self.unbracedSpan.beam.strengthCombos:
                    ...
                    Mmax = abs(max([node.ultM[str(lc)] for node in self.unbracedSpan.nodes], key = abs))
                    if round(Mmax, 2) == 0 * units.kft:
                        self.val[str(lc)] = 1.0 * units('dimensionless')
                    else:
                        self.val[str(lc)] = (12.5*Mmax) / (2.5*Mmax + 3*MA + 4*MB + 3*MC)
            else:
                for lc in self.unbracedSpan.beam.strengthCombos:
                    self.val[str(lc)] = 1.0 * units('dimensionless')
        ...
        def __str__(self):
            ...
            if Cb == 1:
                ...
                return sf.makeEquation('C_b', '(12.5 *mul* M_max) / (2.5 *mul* M_max + 3 *mul* M_A + 4 *mul* M_B + 3 *mul* M_C)', {'M_{max}': round(span.maxMomentNode.MuMaxAbs['val'].to(units), 1), 'M_A': round(span.node_qtrPt.ultM[maxCombo].to(units), 1), 'M_B': round(span.node_midPt.ultM[maxCombo].to(units), 1), 'M_C': round(span.node_threeQtrPt.ultM[maxCombo].to(units), 1)}, round(Cb, 2), lineBreak = True)
                return completeEqn
    ...
    class Fcr():
        ...
        def updateFactor(self):
            ...
            for lc in self.unbracedSpan.beam.strengthCombos:
                ...
                self.val[str(lc)] = ((Cb*math.pi**2*E)/(Lb/rts)**2)*(1+0.078*((J*c)/(Sx*h0))*(Lb/rts)**2)**(1/2)
        ...
        def __str__(self):
            ...
            results = results.replace(f'r_{{ts}}', str(round(self.unbracedSpan.beam.rts.val, 1)))
            ...
            return sf.fixUnits(f'F_{{cr}} & = {variables} \\\\ & = {results} = _bf_{{{val}}}')
    ...
    class phiMn():
        ...
        def updateFactor(self):
            ...
            if self.beam.flange.compactness == 'Slender':
                sys.exit('ERROR: Input section has slender flanges and program cannot perform code checks for this condition.')
            if self.beam.web.compactness != 'Compact':
                sys.exit('ERROR: Input section has noncompact or slender web and program cannot perform code checks for this condition.')
            ...
            if Lb <= Lp:
                ...
                if self.beam.flange.compactness == 'Compact':
                    ...
                    self.consideredLimitStates = 'LTB (not to exceed capacity based on yielding)'
                    if ls0 < ls1:
                        self.val = ls0
                        self.limitState = 'yielding'
                        self.ref = self.beam.Mp.ref
                ...
                if self.beam.flange.compactness == 'Compact':
                    ...
                    self.consideredLimitStates = 'inelastic LTB (not to exceed capacity based on yielding)'
                    if ls0 < ls2:
                        self.val = ls0
                        self.limitState = 'yielding'
                        self.ref = self.beam.Mp.ref
        ...
        def __str__(self):
            ...
            results = self.equation.replace(f'M_p', f'{round(self.beam.Mp.val.to(units.kft), 1)}')
            ...
            results = results.replace(f'F_{{cr}}', f'{round(span.Fcr.val[maxCombo].to("ksi"), 1)}')
            results = results.replace(f'\\lambda_p', str(round(self.beam.flange.lamP.val, 1)))
            results = results.replace(f'\\lambda_r', str(round(self.beam.flange.lamR.val, 1)))
            results = results.replace(f'\\phi_b', str(self.phiB.val))
            results = results.replace(f'\\lambda', str(round(self.beam.flange.lam.val, 1)))
    ...
    def getQuarterPointNodes(self):
        ...
        getActualNode(int(idealQtrPt / self.beam.avgNodeSpacing), idealQtrPt, 'node_qtrPt')
        getActualNode(int(idealMidPt / self.beam.avgNodeSpacing), idealMidPt, 'node_midPt')
        getActualNode(int(idealThreeQtrPt / self.beam.avgNodeSpacing), idealThreeQtrPt, 'node_threeQtrPt')
    ...
    def checkCantilever(self):
        ...
        if self.iNode.condition not in supports and self.iNode.loc == self.beam.nodes[0].loc:
            self.cantilever = True
        elif self.jNode.condition not in supports and self.jNode.loc == self.beam.nodes[-1].loc:
            self.cantilever = True
        else:
